...
template <typename ShapeType>
void Renderer::renderShapeArrays(std::vector<std::array<ShapeType, 3>>& arraysToRender)
{
    for (const auto& shapeArray : arraysToRender)
    {
        for (const auto& shape : shapeArray)
        {
            if (shape.isShown())
            {
                m_window.get().draw(shape.getSprite());
            }
        }
    }
}

template void Renderer::renderShapeArrays(std::vector<std::array<Circle, 3>>&);
template void Renderer::renderShapeArrays(std::vector<std::array<Rectangle, 3>>&);
template void Renderer::renderShapeArrays(std::vector<std::array<Line, 3>>&);
